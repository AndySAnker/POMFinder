...
def get_POMFinder():
    ...
    POMFinder.load_model(cwd+'/Backend/XGBoost_443structures_100PDFperStructure_xPDF_hypercube_sampling_Grmax.model')
...
def PDF_Preparation(Your_PDF_Name, Qmin, Qmax, Qdamp, rmax=30, nyquist="No", plot=True):
    cwd = os.getcwd()
    for i in range(1000):
        with open(cwd+"/Experimental_Data/" + Your_PDF_Name, "r") as file:
            data = file.read().splitlines(True)
            if len(data[0]) == 0:
                with open(cwd+"/Experimental_Data/" + Your_PDF_Name, 'w') as fout:
            ...
            if len(first_line) > 3 and re.match(r'^-?\d+(?:\.\d+)?$', first_line[0]) != None and re.match(r'^-?\d+(?:\.\d+)?$', first_line[1]) == None and re.match(r'^-?\d+(?:\.\d+)?$', first_line[2]) != None:
                PDF = np.loadtxt(cwd+"/Experimental_Data/" + Your_PDF_Name)
                break
            else:
                with open(cwd+"/Experimental_Data/" + Your_PDF_Name, 'w') as fout:
                    fout.writelines(data[1:])
